...
| `scripts/` | DO NOT MODIFY | Index update script and utilities || `scripts/create_note.py` | EXECUTE ONLY | CLI to create a validated note file in `notes/` |
| `scripts/create_question.py` | EXECUTE ONLY | CLI to create a validated question file in `notes/questions/` |
| `scripts/models.py` | DO NOT MODIFY | Shared Pydantic validation models — single source of truth |
## Security Rules
...
5. Agents MUST create notes via `uv run scripts/create_note.py` and questions via `uv run scripts/create_question.py` — never hand-write these files
6. After creating files, agents MUST register them by running: `uv run scripts/update_index.py` — the script scans for all unregistered notes and renames them to `{ID}.md`
7. Notes go in `notes/`, questions go in `notes/questions/`
